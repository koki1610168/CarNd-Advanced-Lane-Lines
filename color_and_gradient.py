...
class ColorAndGradient:
    ...
    def combined(self):
        gradx = self.abs_sobel_thresh(orient='x', thresh_min=12, thresh_max=200) 
        grady = self.abs_sobel_thresh(orient='y', thresh_min=12, thresh_max=200) 
        mag_binary = self.mag_thresh(sobel_kernel=3, mag_thresh=(140, 200))
        dir_binary = self.dir_threshold(sobel_kernel=3, thresh=(np.pi/6, np.pi/2))
        ...
        color_gradient_combined[(gradient_binary == 1) | (color_binary == 1)] = 1
